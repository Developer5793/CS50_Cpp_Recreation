...
void lock_pairs(int& winners, int& losers)
{
    ...
    /*Filling the Rankingsmatrix with Scores According to wikipedia   ---     https://en.wikipedia.org/wiki/Ranked_pairs*/
    int c = 0;
    int d = 0;
    for (int i = 0; i < p; i++)
    {
    ...
    for (int i = 0; i < p; i++)
    {
        for (int j = 0; j < k; j++)
        {
            for (int o = 0; o < h; o++)
            {
                ...
                if (Pairstocheck[o][3].PairstoLock == WinnersandLosersarr[j][4] && Pairstocheck[o][4].PairstoLock == WinnersandLosersarr[i][5])
                {
                    Electionarray[j][i] = 1;
                }
                if (Pairstocheck[o][3].PairstoLock != WinnersandLosersarr[j][4] && Pairstocheck[o][4].PairstoLock != WinnersandLosersarr[i][5] && Electionarray[j][i] != 1)
                {
                    Electionarray[j][i] = 0;
                }
                else if (WinnersandLosersarr[j][4] == WinnersandLosersarr[i][5])
    ...
    cout << endl;
    /*Define exeptions  https://en.wikipedia.org/wiki/Ranked_pairs */
    for (int g = 0; g < h; g++)
    {
        if (Pairstocheck[g][3].PairstoLock == Pairstocheck[g + 1][4].PairstoLock && Pairstocheck[g][4].PairstoLock == Pairstocheck[g + 2][3].PairstoLock && Pairstocheck[g + 1][3].PairstoLock == Pairstocheck[g + 2][4].PairstoLock &&
            Pairstocheck[g][5].voternumber > Pairstocheck[g + 1][5].voternumber && Pairstocheck[g][5].voternumber > Pairstocheck[g + 2][5].voternumber && Pairstocheck[g + 1][5].voternumber > Pairstocheck[g + 2][5].voternumber)
        {
            swap(Pairstocheck[g + 2][3].PairstoLock, Pairstocheck[g + 2][4].PairstoLock);
        }
    }

    /*Build Matrix with 1 for winner and -1 for losers*/
    ...
    for (int r = 0; r < k; r++)
    {
        for (int v = 0; v < p; v++)
        {
            for (int l = 0; l < h; l++)
            {
                if (Pairstocheck[l][3].PairstoLock == WinnersandLosersarr[r][4] && Pairstocheck[l][4].PairstoLock == WinnersandLosersarr[v][5])
                {
                    Electionarray[r][v] = 1;
                }

                if (Pairstocheck[l][3].PairstoLock == WinnersandLosersarr[v][4] && Pairstocheck[l][4].PairstoLock == WinnersandLosersarr[r][5])
                {
                    Electionarray[r][v] = -1;
                }

                if (WinnersandLosersarr[r][4] == WinnersandLosersarr[v][5])
                {
                    Electionarray[r][v] = 0;
                }
            }
    ...
    for (int z = 0; z < p; z++)
    {
        cout << setw(8) << right << WinnersandLosersarr[z][5];
    }
    cout << endl;
    for (int j = 0; j < k; j++)
    {
        cout << setw(10) << left << WinnersandLosersarr[j][4];

        for (int i = 0; i < p; i++)
        {
            cout << setw(8) << right << Electionarray[j][i];
        }
        cout << endl;
    }
}
...
void print_winner(void)
{
    ...
    for (int j = 0; j < winners; j++)
    {
        ...
        {
            if (Electionarray[j][i] == 1)
            {
                Fill_with_1_array[wincounter]++;
            }
        }
        wincounter++;
    }
    
    int pos = 0, n = wincounter;
    ...
    {
        pos = i;
        for (int j = i + 1; j < wincounter; j++)
        {
            if (Fill_with_1_array[pos] < Fill_with_1_array[j]) 
            {
                pos = j;
            }
        }
        swap(Fill_with_1_array[pos], Fill_with_1_array[i]);
        swap(WinnersandLosersarr[pos][4], WinnersandLosersarr[i][4]);
    }

    /*for (int z = 0; z < wincounter; z++)
    {
        cout << Fill_with_1_array[z] << endl;
    }
    */

    std::cout << "The Winner is: " << WinnersandLosersarr[0][4];
