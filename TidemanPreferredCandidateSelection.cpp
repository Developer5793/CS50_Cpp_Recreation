...
// Sort pairs to count who won the most direct comparisons then 
void sort_pairs(void)
{
    ...
    //Count voices
    for (int g = 0; g < winnerslosersarrlenght; g++)
    {        //Works but tries to access the -1 place of the array. How to fix this?
        if (WinnersandLosersarr[g][0] == WinnersandLosersarr[g + 1][0] && WinnersandLosersarr[g-1][0] != WinnersandLosersarr[g][0] || WinnersandLosersarr[g][0] != WinnersandLosersarr[g - 1][0] && WinnersandLosersarr[g][0] != WinnersandLosersarr[g + 1][0] || WinnersandLosersarr[g][0] == WinnersandLosersarr[winnerslosersarrlenght - 1][0]&&WinnersandLosersarr[g][0] != WinnersandLosersarr[g - 1][0])
        {
            for (int q = g; q < winnerslosersarrlenght; q++)
            {
                if (WinnersandLosersarr[g][0] == WinnersandLosersarr[q][0])
                {
                    Rankingsarray[z]++;
 ...
 //3rd column with voices
        for (int z = 0; z < winnerslosersarrlenght; z++)
        {
            if (WinnersandLosersarr[z][0] == WinnersandLosersarr[z+1][0])
            {
                WinnersandLosersarr[z][2] = ss.str()[k];
            }
            else if (WinnersandLosersarr[z][0] != WinnersandLosersarr[z + 1][0] || WinnersandLosersarr[z + 1][0]<= WinnersandLosersarr[winnerslosersarrlenght-1][0])
            {
                WinnersandLosersarr[z][2] = ss.str()[k];
                k++;
            }
   ...
   for (int h = 0; h < winnerslosersarrlenght ; h++)
   {
       ...
       cout << " ";
       cout << WinnersandLosersarr [h][2] << endl;
   }
    return;
}
