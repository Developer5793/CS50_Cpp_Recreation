...
string WinrsLrsArrResult[200][200];
...
int main()
{
    ...
    //Sorting wrong because wrong Memory Management //Call add pairs function with timer to main next// and adjust memory
    for (int pp = 0; pp < 50; pp++)
    {
        cout << WinrsLrsArrResult[pp][0];
        cout << "  " << WinrsLrsArrResult[pp][1];
        cout << endl;
    }
    cout << endl;
    sort_pairs();
...
void add_pairs()
{
    ...
    //Filling up one common array with all the voters Candidate Preference Arrays in order to sort it later on
    for (int o = 0; o <= winnerslosersarrlenght - 1; o++)
    {
        WinrsLrsArrResult[winnerslrsarrlenghtres][0] = WinnersandLosersarr[o][0];
        WinrsLrsArrResult[winnerslrsarrlenghtres][1] = WinnersandLosersarr[o][1];
        winnerslrsarrlenghtres++;
    }
    return;
}
...
void sort_pairs(void)
{
    for (int u = 0; u <= 20; u++)
    {
        for (int q = u + 1; q <= 20; q++)
        {
            if (WinrsLrsArrResult[u][0] != WinrsLrsArrResult[q][0] && WinrsLrsArrResult[u][1] != WinrsLrsArrResult[q][1] &&
                WinrsLrsArrResult[u][0] == WinrsLrsArrResult[q + 1][0] && WinrsLrsArrResult[u][1] == WinrsLrsArrResult[q + 1][1])
            {
                swap(WinrsLrsArrResult[q][0], WinrsLrsArrResult[q + 1][0]);
                swap(WinrsLrsArrResult[q][1], WinrsLrsArrResult[q + 1][1]);
            }
        }
    }
